...
export default class ProgramSettingsService {
  ...
  async initProgramSettingsAccount() {
    let secretKey = Buffer.from(
      JSON.parse(require("fs").readFileSync("key.json", { encoding: "utf-8" }))
    );
  ...
  async retriveProgramSettingsAccount() {
    const programSettings = await this.program.account.programSettings.fetch(
      SNF_PROGRAM_SETTINGS
    );
    console.log("App Settings Account");
    console.log(
      "SNF Foundation Key: ",
      programSettings.snfFoundation.toBase58()
    );
    ...
    console.log(
      "Operator to check index: ",
      programSettings.operatorToCheckIndex
    );
  ...
  async checkOperator() {
    let programSettings = new PublicKey(
      "69UVTJVEyhKQvCC6wGzddH4zmNUs7nEQrryRoP7umZGL"
    );
