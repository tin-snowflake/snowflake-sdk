...
export const HomeViewOld = () => {
    ...
    async function executeFlow() {
        ...
        let targetProgramMetas = flow.actions.reduce((result, current) => result.concat(
                {
    ...
    async function test() {
        const [authority, nonce] = await PublicKey.findProgramAddress(
            [new PublicKey("EERvEgbjiGnBxEpPz4XeNMWEpAgTevXDpBksTMsbNFFA").toBuffer()], programIds().snowflake
        ); // replace by snowflake program id
    ...
    async function createPool() {
        ...
        const [authority, nonce] = await PublicKey.findProgramAddress(
            [snowflakeAccount.publicKey.toBuffer()], programIds().snowflake
        ); // replace by snowflake program id
        ...
        const splHoldingAccount =
            createSplAccount(instructions, walletCtx.publicKey, accountTokenRentExempt, new PublicKey(SOL_MINT_ADDRESS_DEV_NET), authority, AccountLayout.span);
