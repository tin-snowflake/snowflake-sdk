...
import { Keypair, clusterApiUrl, Connection, PublicKey, Transaction, TransactionInstruction, Account } from '@solana/web3.js';
...
export function ConnectionProvider({ children = undefined as any }) {
  ...
  const sendConnection = useMemo(() => new Connection(endpoint, 'confirmed'), [endpoint]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let list = res.filterByChainId(chain.chainID).excludeByTag('nft').getList();
      list.forEach(function (item) {
        if (item.address == WRAPPED_SOL_MINT.toString()) {
...
export const sendTransaction = async (connection: Connection, wallet: WalletContextState, instructions: TransactionInstruction[], signers: Account[], awaitConfirmation = true) => {
  if (!wallet?.publicKey) {
